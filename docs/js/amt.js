...
var AMT = function () {
  ...
  var STORE = "https://java-dev.rgzm.de/rdf4j-server/repositories/amtlimes2";
  var PREFIX = "http://academic-meta-tool.xyz/vocab#";
  var PREFIX_INSTANCES = "http://leiza-scit.sparqlunicorn.cloud/amtlimes/";
  ...
  var loadGraph = function (callback) {
    ...
    // load roles
    queryStore(
      "SELECT ?role ?label ?domain ?range WHERE { ?role rdf:type amt:Role . ?role rdfs:label ?label . ?role rdfs:domain ?domain . ?role rdfs:range ?range . } ORDER BY ASC(?label)",
      function (data) {
        ROLES = data;
        --todo;
        if (todo == 0 && callback) {
